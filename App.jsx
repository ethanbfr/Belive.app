...
CRÉATEUR : ${c?.name||”***”} | ${c?.email||”***”} | ${c?.phone||”___”}
...
const css=`@import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Manrope:wght@300;400;500;600;700;800&display=swap'); *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;-webkit-font-smoothing:antialiased;} body{background:${D};color:white;font-family:'Manrope',sans-serif;} input,select,textarea,button{font-family:'Manrope',sans-serif;} input::placeholder,textarea::placeholder{color:rgba(255,255,255,0.18);} select option{background:#1a1a1a;color:white;} ::-webkit-scrollbar{width:3px;} ::-webkit-scrollbar-thumb{background:rgba(212,16,63,0.3);border-radius:2px;} @keyframes fadeUp{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}} @keyframes slideIn{from{transform:translateX(-100%)}to{transform:translateX(0)}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.3}} .fade{animation:fadeUp 0.3s ease both;} .slide{animation:slideIn 0.25s ease both;} .blink::after{content:'▋';animation:pulse 0.8s infinite;margin-left:2px;} .desktop-sidebar{display:flex;} .mobile-bar{display:none;} @media(max-width:900px){ .desktop-sidebar{display:none !important;} .mobile-bar{display:flex !important;} .main-content{margin-left:0 !important;padding-top:64px !important;} }`;
...
if(u&&u.password===authPass){setUser({email:authEmail,…u});askNotifPermission();return;}
const sv=JSON.parse(localStorage.getItem(“ba6_users”)||”{}”);
if(sv[authEmail]&&sv[authEmail].password===authPass){setUser({email:authEmail,…sv[authEmail]});return;}
...
if(code){
const f=codes.find(c=>c.code===code.toUpperCase()&&!c.usedBy);
if(!f){alert(“❌ Code invalide ou déjà utilisé.”);return;}
r2=f.type;
// Code Créateur Belive → tarif réduit 9,99€
if(f.freeType===“belive_creator”) plan=“belive_creator”;
// Code accès illimité → Pro permanent
if(f.freeType===“unlimited”) plan=“pro”;
// Code limité → période gratuite custom
if(f.freeType===“limited”&&f.freeDays) trialDaysFromCode=f.freeDays;
setCodes(p=>p.map(c=>c.code===f.code?{…c,usedBy:name,usedAt:new Date().toISOString()}:c));
}
...
setUser(p=>({…p,twitch:pseudo,twitchId:twitchUser.id,twitchAvatar:twitchUser.profile_image_url}));
setMs(p=>({…p,twitch:followers}));
alert(`✅ Twitch @${pseudo} connecté ! ${followers.toLocaleString()} followers récupérés.`);
}catch(e){
...
function addStream(){const{date,dur,viewers,platform}=newSt;if(!date||!dur){alert(“Date et durée obligatoires.”);return;}setStreams(p=>[…p,{id:Date.now(),date,duration:parseFloat(dur),viewers:parseInt(viewers)||0,platform}]);setNewSt({date:””,dur:””,viewers:””,platform:“twitch”});setModal(null);}
...
function genCode(type, freeType=“limited”, freeDays=14){
// Code très long et complexe — impossible à deviner
...
type,
freeType,           // “limited” = X jours | “unlimited” = Pro permanent
freeDays: freeType===“limited”?freeDays:null,
createdAt:new Date().toISOString(),
expiresAt:null,     // Pas d’expiration sur le code lui-même
usedBy:null,
usedAt:null,
owner:user.name,
};
    ...
    {page==="dashboard"&&(
      <div>
        ...
        {role==="createur"&&(<>
          ...
          {/* Platforms connect */}
          <Card style={{marginBottom:16}}>
            <div style={{fontWeight:800,marginBottom:12}}>🔗 Connecte tes plateformes</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <div style={{background:"rgba(145,70,255,0.08)",border:`1px solid rgba(145,70,255,0.2)`,borderRadius:12,padding:14,display:"flex",alignItems:"center",gap:10}}>
                <span style={{fontSize:24}}>🟣</span>
                <div style={{flex:1}}><div style={{fontWeight:700,fontSize:13}}>Twitch</div><div style={{fontSize:11,color:M}}>{user.twitch?`@${user.twitch}`:"Non connecté"}</div></div>
              ...
              <div style={{background:"rgba(255,0,0,0.08)",border:`1px solid rgba(255,0,0,0.2)`,borderRadius:12,padding:14,display:"flex",alignItems:"center",gap:10}}>
                ...
                <div style={{flex:1}}><div style={{fontWeight:700,fontSize:13}}>YouTube</div><div style={{fontSize:11,color:M}}>{user.youtube||"Non connecté"}</div></div>
                <Btn sz="sm" v={user.youtube?"success":"ghost"} onClick={connectYoutube}>{user.youtube?"✓":"Connecter"}</Btn>
              </div>
            </div>
            <div style={{marginTop:10,fontSize:11,color:M}}>💡 Une fois connecté, tes infos sont sauvegardées pour toutes tes candidatures partenariats.</div>
          </Card>
          {/* Affiliation */}
          <Card style={{marginBottom:16}}>
            ...
            <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
              {[{t:"createur",l:"🎮 Créateur"},{t:"agent",l:"🎯 Agent"}].map(x=>(
                <div key={x.t} style={{flex:1,minWidth:180}}>
                  ...
                  <Btn full sz="sm" onClick={()=>genCode(x.t)}>{x.l} — Générer</Btn>
                </div>
              ))}
            </div>
          </Card>
        ...
        <div>
          ...
          <Card style={{marginBottom:20}}>
            ...
            <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:12}}>
              <Btn full onClick={()=>genCode(codeType,freeType,freeDays)} icon="🔄">
          ...
          {codes.length>0&&(
            <Card>
              ...
              {[...codes].reverse().map((c,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:`1px solid rgba(255,255,255,0.04)`,flexWrap:"wrap"}}>
                  <div style={{fontFamily:"monospace",color:R,letterSpacing:1,fontWeight:800,fontSize:11,flex:1,minWidth:200}}>{c.code}</div>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                    <Pill color={c.type==="createur"?"purple":"blue"} xs>{c.type}</Pill>
                    {c.freeType==="unlimited"
                      ?<Pill color="green" xs>♾️ Pro permanent</Pill>
                      :<Pill color="yellow" xs>⏳ {c.freeDays||14}j</Pill>
                    }
                    {c.usedBy
                      ?<Pill color="red" xs>✓ {c.usedBy}</Pill>
                      :<Pill color="green" xs>Dispo</Pill>
                    }
                  </div>
                  <button onClick={()=>{navigator.clipboard?.writeText(c.code).catch(()=>{});}} style={{background:"none",border:`1px solid ${B}`,borderRadius:6,padding:"3px 8px",color:M,fontSize:10,cursor:"pointer"}}>📋</button>
