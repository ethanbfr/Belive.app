...
export default function App(){
  ...
  function connectTwitch(){const pseudo=prompt("Entre ton pseudo Twitch :");if(!pseudo)return;setUser(p=>({...p,twitch:pseudo}));setMs(p=>({...p,twitch:parseInt(prompt("Combien de followers Twitch as-tu ?")||"0")}));alert(`✅ Twitch @${pseudo} connecté ! Tes stats ont été mises à jour.`);}
  function connectYoutube(){const ch=prompt("Entre le nom de ta chaîne YouTube :");if(!ch)return;setUser(p=>({...p,youtube:ch}));setMs(p=>({...p,youtube:parseInt(prompt("Combien d'abonnés YouTube as-tu ?")||"0")}));alert(`✅ YouTube "${ch}" connecté !`);}
  ...
  return(
    <div style={{display:"flex",minHeight:"100vh",background:D}}>
            ...
            {role==="createur"&&(<>
              ...
              <Card>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:10,marginBottom:10}}>
                  ...
                  <Field label="Plateforme" as="select" value={newSt.platform} onChange={e=>setNewSt({...newSt,platform:e.target.value})}><option value="twitch">🟣 Twitch</option><option value="youtube">▶️ YouTube</option><option value="tiktok">🎵 TikTok</option></Field>
                </div>
      ...
      <Modal open={modal==="addSc"} onClose={()=>setModal(null)} title="Planifier un stream">
        ...
        <Field label="Plateforme" as="select" value={newSc.platform} onChange={e=>setNewSc({...newSc,platform:e.target.value})}><option value="twitch">🟣 Twitch</option><option value="youtube">▶️ YouTube</option><option value="tiktok">🎵 TikTok</option></Field>
